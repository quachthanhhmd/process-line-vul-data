...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', default='devign',
                        choices=['chrome_debian/balanced', 'chrome_debian/imbalanced', 'chrome_debian', 'devign'])
    parser.add_argument('--features', default='ggnn',
                        choices=['ggnn', 'wo_ggnn'])
    ...
    parser.add_argument('--baseline_model', default=None)  # 'svm')
    ...
    if feature_name == 'ggnn':
        if dataset == 'chrome_debian/balanced':
            ...
            ds = "/home/p9chakra/MY_DRIVES/ProgramFiles/Devign/input_dir/"
    ...
    output_file_name = output_dir + '/' + \
        dataset.replace('/', '_') + '-' + feature_name + '-'
    if args.lambda1 == 0:
        assert args.lambda2 == 0
        output_file_name += 'cross-entropy-only-layers-' + \
            str(args.num_layers) + '.tsv'
    else:
        output_file_name += 'triplet-loss-layers-' + \
            str(args.num_layers) + '.tsv'
    ...
    for _ in range(30):
        ...
        print(train_X.shape, train_Y.shape, test_X.shape,
              test_Y.shape, sep='\t', file=sys.stderr, flush=True)
        if args.baseline:
            model = SVMLearningAPI(
                True, args.baseline_balance, model_type=args.baseline_model)
        else:
            model = RepresentationLearningModel(
                ...
                num_layers=args.num_layers, num_epoch=13
            )
            if os.path.exists("Vuld_SySe/representation_learning/TrainedModels/rep_model.bin"):
