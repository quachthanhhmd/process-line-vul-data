...
def train(model, dataset, optimizer, num_epochs, max_patience=5,
    ...
    assert isinstance(model, MetricLearningModel) and isinstance(
        dataset, DataSet)
    best_f1 = 0
    best_model = None
    patience_counter = 0
    ...
    try:
        for epoch_count in range(num_epochs):
            ...
            if output_buffer is not None:
                ...
                print('After epoch %2d Train loss : %10.4f' %
                      (epoch_count, epoch_loss), file=output_buffer)
                print('=' * 100, file=output_buffer)
            if epoch_count % valid_every == 0:
                ...
                if vf1 > best_f1:
                    ...
                    torch.save(best_model, "TrainedModels/rep_model.bin")
                else:
                    patience_counter += 1
    ...
    with torch.no_grad():
        ...
        for _ in batch_generator:
            ...
            batch_pred = np.argmax(
                probs.detach().cpu().numpy(), axis=-1).tolist()
