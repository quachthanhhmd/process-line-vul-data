...
def inputGeneration(nodeCSV, edgeCSV, target, wv, edge_type_map, cfg_only=False, return_value=None, index=None):
    gInput = dict()
    gInput["targets"] = list()
    gInput["graph"] = list()
    ...
    with open(nodeCSV, 'r') as nc:
        ...
        for idx, node in enumerate(nodes):
            ...
            nodeKey = node['key']
            node_type = node['type']
            if node_type == 'File'or node_type not in type_map.keys():
                continue
...
def main():
    ...
    parser.add_argument('--csv', help='normalized csv files to process', default='/home/p9chakra/MY_DRIVES/ProgramFiles/evaluating-deep-learning-based-vulnerability-detection-models-on-realistic-datasets/project/parsed/temp_dir/source_code')
    parser.add_argument('--src', help='source c files to process', default='/home/p9chakra/MY_DRIVES/ProgramFiles/evaluating-deep-learning-based-vulnerability-detection-models-on-realistic-datasets/project/temp_dir/source_code')
    parser.add_argument('--wv', default='/home/p9chakra/MY_DRIVES/ProgramFiles/ReVeal/wv_models_comp_vul_100/code.bin')
    parser.add_argument('--output', default='compvul_ggnn.json')
    args = parser.parse_args()
    nltk.download('punkt')
    json_file_path = '/home/p9chakra/MY_DRIVES/ProgramFiles/ReVeal/data/comp_vul_full_data_with_slices.json'
    ...
    for didx, entry in enumerate(tqdm(data)):
        ...
        return_values = [None]*4
        thread_one = threading.Thread(target=inputGeneration, args=(nodes_path, edges_path, label, model, edgeType_full, False, return_values,0))
        thread_one.start()
        thread_two = threading.Thread(target=inputGeneration,args=(nodes_path, edges_path, label, model, edgeType_control, True, return_values, 1))
        thread_two.start()
        thread_three = threading.Thread(target=inputGeneration,args =(nodes_path, edges_path, label, model, edgeType_data, True, return_values, 2))
        thread_three.start()
        thread_four = threading.Thread(target=inputGeneration,args=(nodes_path, edges_path, label, model, edgeType_control_data, True, return_values, 3))
        thread_four.start()
        ...
        thread_three.join()
        thread_four.join()
        graph_input_full = return_values[0]
        graph_input_control = return_values[1]
        graph_input_data = return_values[2]
        graph_input_cd = return_values[3]
