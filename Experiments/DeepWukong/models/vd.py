...
class DeepWuKong(LightningModule):
    ...
    def forward(self, batch: Batch) -> torch.Tensor:
        ...
        # [n_XFG; n_classes]
        return hiddens, self.__classifier(hiddens)
    ...
    def _shared_epoch_end(self, step_outputs: EPOCH_OUTPUT, group: str):
        if group in ["test","val"]:
            file_paths,preds,labels=[],[],[]
            ...
            df=pd.DataFrame({"file_path":file_paths,"pred":preds,"label":labels})
            df.to_csv(join(self.__config.root_folder_path, self.__config.split_folder_name,group+"_results.csv"),index=False)
        ...
        preds = torch.cat([tmp['preds'] for tmp in step_outputs]).tolist()
        targets = torch.cat([tmp['target'] for tmp in step_outputs]).tolist()
